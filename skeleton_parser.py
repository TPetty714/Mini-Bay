...
def writeItem(item):
    ...
    numberOfBids = getElementTextByTagNameNR(item, 'Number_of_Bids')
    writeLine(item_file, [itemID,'\"'+name+'\"', currently, buy_price, first_bid, started, ends, sellerID, '\"'+description+'\"', numberOfBids])
...
def writeBid(bid, itemID):
    ...
    bidTime = transformDttm(getElementTextByTagNameNR(bid, 'Time'))
    amount = transformDollar(getElementTextByTagNameNR(bid, 'Amount'))
    writeLine(bid_file, [itemID, userID, bidTime, amount])
